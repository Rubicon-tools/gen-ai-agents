...
export default function ChatPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {messages.length > 0 && (
        <PageHeader
          title="Chat IA AgriTech"
          description="Assistant IA spécialisé en agriculture et technologies agricoles"
      ...
      <div className="container mx-auto px-6 py-16">
        <div className="max-w-4xl mx-auto">
          {messages.length === 0 ? (
            <div className="text-center py-16">
              <div className="mb-16">
                ...
                <p className="text-[#6C757D] text-lg font-medium">
                  Utilisez l'une des suggestions ci-dessous ou posez votre propre question
                </p>
              </div>
                ...
                {suggestedPrompts.map((prompt, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="w-full h-[120px] bg-[#E9F5EC] rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-green-100/50 hover:bg-[#E9F5EC]/80 group border-0"
                    >
                      <div className="flex flex-col h-full">
                        <div className="mb-3">
                          <IconComponent className="w-6 h-6 text-[#2D6A4F] group-hover:text-[#1B4332] transition-colors" />
                        </div>
              ...
              <div className="mb-12">
                <Button
                  variant="ghost"
                  className="text-[#6C757D] hover:text-[#2D6A4F] text-sm font-medium hover:bg-[#E9F5EC]/50 rounded-xl px-6 py-3 transition-all duration-200"
                >
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg border-0 p-8 mb-8">
              <div className="space-y-6 max-h-96 overflow-y-auto">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-4 ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                    {msg.sender === "ai" && (
                      <div className="w-10 h-10 bg-gradient-to-br from-[#1B4332] to-[#2D6A4F] rounded-full flex items-center justify-center flex-shrink-0 shadow-md">
                    ...
                    <div
                      className={`max-w-[75%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${
                        msg.sender === "user"
                          ...
                          : "bg-[#E9F5EC] text-[#1B4332] border border-green-100"
                      }`}
                    >
                      {msg.content}
                    </div>
          ...
          <div className="max-w-[900px] mx-auto">
            <form onSubmit={handleSubmit} className="relative">
              <div className="flex items-center gap-4 min-h-[52px] bg-white/90 backdrop-blur-sm rounded-3xl p-4 shadow-lg -mt-24">
                <div className="flex items-center gap-3">
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    className="w-10 h-10 text-[#6C757D] hover:text-[#2D6A4F] hover:bg-[#E9F5EC]/50 rounded-xl transition-all duration-200"
                  >
                ...
                <Input
                  ...
                  placeholder="Posez votre question sur l'AgriTech…"
                  className="flex-1 border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-[#6C757D] text-base px-0 h-[52px] text-[#1B4332] shadow-none"
                  disabled={isLoading}
                />
                ...
                <Button
                  ...
                  className="bg-[#1B4332] hover:bg-[#2D6A4F] text-white rounded-full w-10 h-10 shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50"
                >
